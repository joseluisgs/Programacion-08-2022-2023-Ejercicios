...
object ModuloFileXml: ModuloStorageService {
    ...
    override fun saveAll(elements: List<Modulo>): List<Modulo> {
        ...
        if (file.exists() && !file.canWrite()) throw IOException("ERROR: ModuloFileXml ->\tNo se puede escribir en el fichero XML")
    ...
    override fun loadAll(): List<Modulo> {
        val file = File(localPath)
        if (!file.exists() || !file.canRead()) throw IOException("ERROR: ModuloFileXml ->\tNo se puede leer en el fichero XML")
        val persister = Persister()
