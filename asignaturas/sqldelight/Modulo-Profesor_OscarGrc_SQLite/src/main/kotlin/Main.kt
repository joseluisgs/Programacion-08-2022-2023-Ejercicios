...
import services.storage.profesor.ProfesorStorage
...
// ahora el controlador Conductores
    ...
    var profesor: Profesor = controller.findById(2).getOrElse {
        throw IllegalStateException("Error al obtener el profesor: $it")
    }
    var otroProfesor: Profesor = controller.findById(3).getOrElse {
        throw IllegalStateException("Error al obtener el profesor: $it")
    }
    println(controller2.save(
        Modulo(uuid = UUID.randomUUID(), nombre = profesor.nombre, curso = 1.toString(), grado = Grado.DAM)))
    println(controller2.save(Modulo(uuid =UUID.randomUUID(),nombre = profesor.nombre,
        curso = 1.toString(),grado = Grado.ASIR)))
    println(controller2.save(Modulo(uuid = UUID.randomUUID(), nombre = otroProfesor.nombre,
        curso = 2.toString(),grado=Grado.DAW)))
    var listamodulos = controller2.findAll()
    println(controller2.findAll())
    println(controller2.findByUUID(listamodulos[0].uuid.toString()) ?: "No existe el usuario")
    println("")
    println("UPDATE")
    println(controller2.findAll())

    println(controller2.update(Modulo(uuid=listamodulos[0].uuid,nombre="Carmen2", curso=1.toString(),grado=Grado.DAM)))
    println(controller2.findAll())
